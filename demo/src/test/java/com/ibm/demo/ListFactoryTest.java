...
public class ListFactoryTest extends TestCase{
	...
	/**
	 * add(E arg0)のテスト--投入したオブジェクトがListFactoryに、addされることを確認する。
	 *
	 * @param
	 *
	 * @return
	...
	/**
	 * add(int arg0, E arg1)のテスト--投入したオブジェクトがListFactoryに、addされることを確認する。
	 *
	 * @param
	 *
	 * @return
	...
	/**
	 * get(int arg0)のテスト-- ListFactoryからオブジェクトを取得できること。
	...
	public void test_iterator(){
		...
		Iterator<TestBean> itr = lst.iterator();
		int cnt = 1;
		while(itr.hasNext()){
	...
	/**
	 * remove(int index)のテスト-- ListFactoryから指定indexのオブジェクトが削除できること。また、指定された位置に以前あった要素 が戻ることを確認する。
	 *
	 * @param
	 *
	 * @return
	...
	//		assertEquals("jp.co.ykkap.common.util.TestBean2 incompatible with jp.co.ykkap.common.util.TestBean", e.getMessage());
		}
		try{
			lst2 = new ArrayList();
			lst2.add(tb2);
	...
	public void test_add_NoSerializable(){
		...
		List<TestBean3> lst = null;
		ListFactory<TestBean3> lf = new ListFactory<TestBean3>();
		lst = lf.createList();
		try{
		lst.add(tb3);
	...
	/**
	 * オブジェクト出力先パスのテスト-- ListFactoryのコンストラクタに、出力先パスを設定した場合、指定したパスに、出力されていることを確認。
	 *
	 * @param
	 *
	 * @return
	...
	public void test_StringTest(){
		...
		ListFactory<String> lf = new ListFactory<String>();
		lst = lf.createList();
	 ...
	 * 複数実行テスト
	 *
	 * 複数実行した場合に、使用中のファイルが削除されてしまわないか確認する
	 *
	 * @param
	 *
	 * @return
	...
	@Test
	//Stringでのテスト
	public void test_FileDeleteTest(){
		List<String> lst = null;
		...
		ListFactory<String> lf2 = new ListFactory<String>();
		lst2 = lf2.createList();

		assertEquals("test1", lst.get(0).toString());

		lst2.add("test2");
		assertEquals("test2", lst2.get(0).toString());
		lst.clear();
		lst2.clear();
	}
}
